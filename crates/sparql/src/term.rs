...
/// One RDF term. Field order gives the same ordering as the C engine's
/// term_cmp (kind, lex, dtype, lang), which the Turtle serializer relies on.
...
/// Datatypes treated as numeric by FILTER comparisons (same set as the C
/// engine). ORDER BY uses the shorter integer/decimal/double/float/long/int
/// set, also matching the C engine.
